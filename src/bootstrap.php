...
use Kdevy\Phpfw\Exception\HttpError;
...
logsave("system:init", "module = {$module_name}, action = {$action_name}");
...
try {
    ...
    if (!isset($action_object)) {
        ...
        <head>
            ...
            <title>" . HTTP_PHRASES[$code] . " ({$code})</title>
            <style>
            body {margin: 20px 30px;}
            </style>
        </head>
        <body>
            <h2 style='font-family: serif;'>" . HTTP_PHRASES[$code] . " ({$code})</h2>
        </body>
        </html>";
        exit();
    }
...
logsave(
    "system:init",
    "exit MU = " . memory_get_usage(true) / 1024 . " kb"
